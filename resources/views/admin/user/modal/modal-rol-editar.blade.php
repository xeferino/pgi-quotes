  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">Editar Rol <img src="{{ asset('img/loading.GIF') }}" alt="" style="width: 32px;margin-left: 10px;display: none;" id="load_cargar_rol_editar"></h4>
      </div>
      <div class="modal-body">
        <div v-if="dataShowModalDataRol.on==true">
                          ...
                          <div class="col-md-10 col-md-offset-1">
                            @if(count($secciones)>0)
                              @foreach($secciones as $seccion)
                                <div class="checkbox">
                                  <label>
                                    <input type="checkbox" id="seccion_{{ $seccion->id }}" v-on:click="onClickCheckSeccion({{ $seccion->id }})"> <small id="sec_{{ $seccion->id }}">{{ $seccion->nombre_seccion }}</small>
                                  </label>
                                </div>
                              @endforeach
                            @else
                              <small style="color:red">No tiene Secciones.</small> <a href="{{ url('dash/seccion/new') }}"><small>Ir a Crear Sección</small></a>
                            @endif
                          </div>
                        </div>
                      </div>
          ...
          <input type="hidden" value="{!! csrf_token() !!}" id="token_edit_rol">
      ...
      <div class="modal-footer">
       <img src="{{ asset('img/loading.GIF') }}" alt="" style="width: 32px;margin-right: 10px; display: none" id="load_editar_rol">
